 ...
 *
 * The satellite-imagery guide (`/knowledge/satellite`) used to seed here
 * too (`SATELLITE_GUIDES`, tenant-scoped — required `npm run
 * import:knowledge` per tenant before it showed up). W5's final task
 * (2026-08-09) promoted it to a GLOBAL article instead — see
 * `scripts/rag/ingest-satellite-guide.ts` — so every tenant gets it for
 * free, with no per-tenant seed step. `ALL_SEED_ARTICLES` below is now
 * just `GROWING_GUIDES`; the name is kept (rather than renamed to
 * `GROWING_GUIDES` everywhere) so `tests/guardrails/dose-phi-rei-gate.test.ts`
 * and `scripts/seed-demo.ts` don't need an unrelated rename alongside
 * this fold.
 *
...
/** Every article this script seeds — growing guides only (the satellite
 *  guide is GLOBAL now; see `scripts/rag/ingest-satellite-guide.ts`). */
export const ALL_SEED_ARTICLES: Guide[] = GROWING_GUIDES;
