/**
 * Duplicate practice↔evidence links, against a real Postgres transaction.
 *
 * The bug this file exists for could not be seen by a unit test, and that is
 * the whole point of it living here.
 *
 * `uploadEvidenceFile` wrapped its `PracticeEvidenceLink` insert in
 ...
 *      acceptance criterion: uploading a duplicate file to a practice succeeds.
 */
...
beforeAll(async () => {
    ...
    const practice = await prisma.practice.create({
        data: { tenantId: TENANT_ID, name: 'Practice receiving duplicate evidence' },
    });
    CONTROL_ID = practice.id;
});
...
afterAll(async () => {
    ...
    try { await prisma.practiceEvidenceLink.deleteMany({ where: scope }); } catch { /* best effort */ }
    ...
    try { await prisma.practice.deleteMany({ where: scope }); } catch { /* best effort */ }
...
describeFn('duplicate practice↔evidence link (integration)', () => {
    it('a swallowed 23505 still poisons the transaction — the premise of the fix', async () => {
        ...
        await prisma.practiceEvidenceLink.create({
            data: { tenantId: TENANT_ID, practiceId: CONTROL_ID, kind: 'LINK', url },
        });

        await expect(
            prisma.$transaction(async (tx) => {
                try {
                    await tx.practiceEvidenceLink.create({
                        data: { tenantId: TENANT_ID, practiceId: CONTROL_ID, kind: 'LINK', url },
                    });
                } catch {
                ...
                await tx.practice.findFirst({ where: { id: CONTROL_ID } });
            }),
    ...
    it('ON CONFLICT DO NOTHING leaves the transaction usable', async () => {
        ...
        const after = await prisma.$transaction(async (tx) => {
            await tx.practiceEvidenceLink.createMany({
                data: [{ tenantId: TENANT_ID, practiceId: CONTROL_ID, kind: 'LINK', url }],
                skipDuplicates: true,
            });
            // The statement that used to die with 25P02.
            return tx.practice.findFirst({ where: { id: CONTROL_ID } });
        });

        expect(after?.id).toBe(CONTROL_ID);
        // Still exactly one link — skipDuplicates skipped, it did not insert.
        const links = await prisma.practiceEvidenceLink.count({
            where: { tenantId: TENANT_ID, practiceId: CONTROL_ID, url },
        });
        expect(links).toBe(1);
    });

    it('uploading the SAME file to the SAME practice twice succeeds', async () => {
        ...
        // (practiceId, kind, fileId) unique constraint — the exact collision.
        const first = await uploadEvidenceFile(adminCtx(), txtFile('same bytes every time'), {
            title: 'First upload',
            practiceId: CONTROL_ID,
        });
        ...
        const links = await prisma.practiceEvidenceLink.count({
            where: { tenantId: TENANT_ID, practiceId: CONTROL_ID, fileId: first.fileRecordId },
        });
        expect(links).toBe(1);
    ...
    it('a duplicate LINK evidence row on the same practice also survives', async () => {
        ...
        const links = await prisma.practiceEvidenceLink.count({
            where: { tenantId: TENANT_ID, practiceId: CONTROL_ID, kind: 'LINK', url: content },
        });
        expect(links).toBe(1);
    });
});
