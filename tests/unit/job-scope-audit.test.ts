...
describe('Executor Registry — tenantId propagation audit', () => {
    ...
    test('no executor silently ignores payload.tenantId', () => {
        ...
        while ((opener = openerRe.exec(registrySource)) !== null) {
            ...
            // news-pull writes the GLOBAL, tenant-agnostic MarketNewsItem cache
            // (no tenantId, like MarketPriceSeries/SoilSample) from env-configured
            // RSS/Atom feeds — no tenant axis by design.
            if (['health-check', 'sync-pull', 'schedule-trigger-sweep', 'sharepoint-delta-sync-dispatch', 'sharepoint-subscription-renew', 'risk-appetite-monitor', 'risk-snapshot', 'report-delivery', 'exchange-expiry-sweep', 'market-prices-pull', 'news-pull'].includes(jobName)) continue;
