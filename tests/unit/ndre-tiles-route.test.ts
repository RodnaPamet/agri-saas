 ...
 *     `{ tileUrl, acquiredDate }` pair to Redis under the `ndre:` cache-key
...
const getNdreTileUrlMock = jest.fn<
...
import { GET } from '@/app/api/t/[tenantSlug]/agro/locations/[locationId]/ndre-tiles/route';
...
function call(qs: string) {
    ...
    const req = new NextRequest(`http://localhost/api/t/acme/agro/locations/loc-1/ndre-tiles?${qs}`);
...
beforeEach(() => {
    ...
    getNdreTileUrlMock.mockResolvedValue({
...
it('generates + caches a tile URL on a cache miss', async () => {
    ...
    expect(redisSet).toHaveBeenCalledWith(
        'ndre:tile:tenant-1:loc-1:clip3:2026-06-15',
