...
describe('GET /api/docs — Swagger UI gating', () => {
    ...
    it('returns 200 + HTML in development', async () => {
        ...
        // #944 — the spec moved OUT of public/ so it could be gated. A file in
        // public/ is served by Next's static handler before any application
        // code runs, so it cannot be gated by middleware, a route, or Caddy.
        // The UI now fetches the session-gated route.
        expect(body).toContain("url: '/api/openapi'");
        expect(body).not.toContain("url: '/openapi.json'");
    });
