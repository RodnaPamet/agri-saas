 ...
 *     `{ tileUrl, acquiredDate }` pair to Redis under the `ndmi:` cache-key
...
const getNdmiTileUrlMock = jest.fn<
...
beforeEach(() => {
    ...
    getNdmiTileUrlMock.mockResolvedValue({
...
it('generates + caches a tile URL on a cache miss', async () => {
    ...
    expect(redisSet).toHaveBeenCalledWith(
        'ndmi:tile:tenant-1:loc-1:clip3:2026-06-15',
