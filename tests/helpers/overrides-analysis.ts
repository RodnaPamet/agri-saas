 ...
 * NEVER silently dropped — from EITHER side of the comparison:
 *
 *   - a REQUESTER's declared range, collected by `requestersFor`, and
 *   - an OVERRIDE's own value, after `$name` resolution, collected in
 *     `analyseOverrides` before C and D read it.
 *
 * Both land in `unparseableRanges` so the guard can assert that list is
 * empty. Both C and D fail toward green on a range they cannot parse (C
 * needs to prove ALL requesters are subsets; D needs to prove a specific
 * relation), which is exactly the shape that rots into a tautology — so
 * the guard treats a non-empty `unparseableRanges` as a failure rather
 * than as a shrug.
 *
 * The override-value half was MISSING until 2026-09-13 and this paragraph
 * claimed otherwise (#866). Measured on the real tree:
 * `"fast-uri": "not-a-parseable-range"` in package.json — an override npm
 * can apply to nothing — left this analysis and BOTH guards over it at
 * 50/50 green, because `resolve()` hands back the raw literal and C and D
 * each `continue` past a value they cannot parse.
 *
 * The one shape deliberately NOT in this list is an unresolvable `$name`
 * (`resolved === null`): it is not a range at all, and the guard asserts
 * on it separately so the two failures do not read as one.
 */
...
export interface Analysis {
    ...
    edges: OverrideEdge[];
    /**
     * Ranges no semver parse could make sense of — override VALUES and
     * requester ranges alike. MUST be empty; see "## Failing safe" above.
     */
...
function isParseableRange(range: unknown): range is string {
...
export function analyseOverrides(pkg: PackageJson, lock: Lockfile): Analysis {
    ...
    // An override VALUE npm cannot parse is not a floor — npm applies
    // nothing — and C and D both `continue` past one, so without this it is
    // dropped in silence. Collected here, once per edge, BEFORE anything
    // reads `edge.resolved`. See "## Failing safe" above for the measurement.
    for (const edge of edges) {
        // `resolved === null` is an unresolvable `$name`, not an unparseable
        // range; the guard asserts on that separately.
        if (edge.resolved === null || isParseableRange(edge.resolved)) continue;
        unparseableRanges.push(
            `${edgeTarget(edge)}: the override VALUE "${edge.raw}"` +
                (edge.raw === edge.resolved ? '' : ` (resolves to "${edge.resolved}")`) +
                ' is not a range semver can parse, so this entry floors nothing',
        );
    }
