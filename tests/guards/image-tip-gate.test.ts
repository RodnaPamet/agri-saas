...
/** A `gh` stub that answers the runs query with the statuses given. */
function ghStub(dir: string, runs: Array<{ status: string; conclusion: string | null }> | 'fail'): string {
    const p = path.join(dir, 'gh');
    const body =
        runs === 'fail'
            ? `#!/bin/bash\necho "gh: HTTP 403: API rate limit exceeded" >&2\nexit 1\n`
            : `#!/bin/bash\ncat <<'JSON'\n${JSON.stringify({
                  workflow_runs: runs.map((r) => ({ name: PUBLISH, status: r.status, conclusion: r.conclusion })),
              })}\nJSON\n`;
...
function runGate(opts: {
    target: string;
    triggering: string;
    runs: Array<{ status: string; conclusion: string | null }> | 'fail';
}): Decision {
