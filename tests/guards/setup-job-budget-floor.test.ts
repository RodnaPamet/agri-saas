...
/*
 * NOTE — "is every `npm ci` bounded?" is NOT asserted here.
 *
 * An earlier draft of this file carried that check. It belongs to
 * `tests/guardrails/ci-retry-loop-timeout.test.ts`, which already owned the
 * invariant ("retry loops cap each attempt") and simply had not been pointed
 * at `.github/actions/`. Extending that guard was the right fix; adding a
 * second one beside it would have created two rules for one property, which
 * drift apart exactly the way the two INSTALL PATHS did.
 *
 * This file owns one thing: a job that installs can outlive its install.
 */
