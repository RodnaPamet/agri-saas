...
// ## Why three of these four take their list as a PARAMETER
//
// Each one is a SELECTION, and an empty selection passes every assertion
// below. The controls that are supposed to separate "this selector works"
// from "this selector returns nothing" therefore have to run the selector
// on the input production actually gives it — the real 20 findings, the
// real lockfile, the real dated lists — with ONE defect injected. A control
// that builds its own 0- or 1-element fixture keys on the input SIZE, and
// #866 measured all four ways that fails: `if (findings.length > 0)
// return []` inside `staleWaivers` and `if (findings.length > 1) return []`
// inside `unwaived` both left the suite fully green, because the live
// assertion always passed 20 and the control always passed 0 or 1.
//
// So the lists are parameters with the live list as the default: the live
// assertions read exactly as before, and the controls pass the real input
// plus the defect.
...
function staleWaivers(findings: Finding[], waivers: Waiver[] = WAIVERS): string[] {
...
function stableDormantFloors(floors: DormantFloor[] = DORMANT_FLOORS, l: Lockfile = lock): string[] {
    return floors
        .filter((d) => {
            const { name, selector } = splitOverrideKey(d.key);
            return lockEntriesFor(l, name, selector).length > 0;
        })
        .map((d) => d.key);
}
 ...
 *
 * This is the selection `expired()` reasons over, so gutting it to
 * `return []` retires the whole dated fuse while leaving 'no expired entry'
 * green — measured in #866, together with a waiver back-dated to 2020, and
 * both states were 71/71. The control is an IDENTITY assertion against the
 * two source lists (below, 'datedEntries really is both lists'): it cannot
 * hold while either list has an entry this function drops.
 */
...
describe('overrides — the analysis is looking at something', () => {
    ...
    it('every override value resolves to a parseable range', () => {
        // The name of this test was true of its subject and false of its
        // assertion: it only ever checked `resolved === null`, so a value
        // that resolved to a STRING npm cannot parse
        // (`"sharp": "not-a-parseable-range"`) walked straight through it —
        // and through C and D, which both `continue` on `!isParseableRange`.
        // Parseability is now asserted, which is what the name always said.
        const broken = analysis.edges.filter((e) => e.resolved === null || !isParseableRange(e.resolved));
        expect(broken.map((e) => `${e.key} > ${e.child} = ${e.raw} -> ${e.resolved}`)).toEqual([]);
    });

    // The two controls below are the ones that were missing entirely: they
    // run the analysis on the REAL package.json and the REAL lockfile with a
    // single unusable value injected, so they fail if the collection stops
    // happening — which is how `unparseableRanges` came to be documented as
    // catching an override's own value while only ever seeing a requester's.
    it('an override value npm cannot parse is COLLECTED, not silently dropped', () => {
        // `"sharp": "not-a-parseable-range"` is the exact injection from
        // #866, measured green across 45/45 before this. `sharp` is chosen
        // because its real value is `$sharp`, i.e. the entry is clean today,
        // so a report on it can only come from the injection.
        const mutated: PackageJson = {
            ...pkg,
            overrides: { ...pkg.overrides, sharp: 'not-a-parseable-range' },
        };
        const mutatedAnalysis = analyseOverrides(mutated, lock);
        expect(mutatedAnalysis.unparseableRanges.filter((r) => r.startsWith('sharp:'))).toHaveLength(1);
        expect(mutatedAnalysis.unparseableRanges.join('\n')).toContain('not-a-parseable-range');
        // and the unmutated tree stays clean, or the assertion above proves
        // only that sharp is always reported.
        expect(analysis.unparseableRanges).toEqual([]);
    });

    it('a `$name` reference to a package the root does not declare is COLLECTED', () => {
        // The defensive-edit shape: `resolve()` returns null here, and one
        // word — `?? null` becoming `?? raw` — used to turn that null into
        // the literal string `$sharp-typo`, which passed
        // `resolved === null` and was then skipped by C and D. Both halves
        // now land in `unparseableRanges`: a null value, and a `$name` that
        // survived as a non-range string.
        const mutated: PackageJson = {
            ...pkg,
            overrides: { ...pkg.overrides, sharp: '$sharp-typo' },
        };
        const mutatedAnalysis = analyseOverrides(mutated, lock);
        expect(mutatedAnalysis.unparseableRanges.filter((r) => r.startsWith('sharp:'))).toHaveLength(1);
        const edge = mutatedAnalysis.edges.find((e) => e.key === 'sharp');
        expect(edge && isParseableRange(edge.resolved)).toBe(false);
    });
});
...
describe('overrides — every structural finding is answered', () => {
    ...
    it('...and staleWaivers selects on the REAL findings — the control on the line above', () => {
        // `expect(stale).toEqual([])` is satisfied by a function that always
        // returns []. The first attempt at a control here asserted
        // `staleWaivers([])` had WAIVERS.length entries, and #866 measured
        // what that was worth: `if (findings.length > 0) return []` — one
        // line, WAIVERS turned into a permanent allowlist on the real tree —
        // passed 71/71 INCLUDING that control, because the control's input
        // was 0 findings and the live assertion's was always 20. The two
        // input sets never overlapped.
        //
        // So this passes the REAL 20 findings, with the defect injected into
        // the WAIVER side: one waiver for a finding the tree does not
        // produce. Exactly that key must come back — under either gut it
        // comes back empty.
        const ghost: Waiver = {
            check: 'C',
            target: 'a-target-no-override-in-this-repo-declares',
            reason: 'Synthetic, never in WAIVERS. Present only as the injected defect for this control.',
            review: '2099-01-01',
        };
        expect(staleWaivers(analysis.findings, [...WAIVERS, ghost])).toEqual([
            'C:a-target-no-override-in-this-repo-declares',
        ]);
        // The complement, and the reason the pair discriminates: over-report
        // is caught by the live assertion above (every real waiver IS live),
        // under-report by this one. A selector can satisfy neither by
        // returning a constant.
        expect(analysis.findings.length).toBeGreaterThan(0);
    });

    it('no unwaived finding — and unwaived() selects on the REAL findings', () => {
        // Companion control to 'no unwaived finding' above, same defect
        // class: `if (findings.length > 1) return []` inside unwaived()
        // passed 71/71, because the only control fed it ONE synthetic
        // finding while the tree always produces 20. The input here is the
        // real 20 PLUS one unwaived finding, so the mutation's own trigger —
        // input size — is on production's side of the line.
        const intruder: Finding = {
            check: 'D',
            target: 'a-target-no-override-in-this-repo-declares',
            detail: 'Synthetic, matched by no WAIVERS or DORMANT_FLOORS entry.',
        };
        const open = unwaived([...analysis.findings, intruder]);
        expect(open.map(findingKey)).toEqual(['D:a-target-no-override-in-this-repo-declares']);
    });
    ...
    it('...and stableDormantFloors selects on the REAL lockfile', () => {
        // `return []` as the first statement passed 71/71 in #866, and so
        // did the same gut with a FALSE dormant floor (`uuid` — an installed
        // override key) added to DORMANT_FLOORS. The lockfile here is the
        // real one, unmodified; the defect is injected into the floors list,
        // which is the side a dormant floor is written on.
        const falseClaim: DormantFloor = {
            key: 'uuid',
            reason:
                'Synthetic, never in DORMANT_FLOORS. `uuid` is a real override key whose package IS ' +
                'installed, so claiming it is dormant is exactly the false claim this rule exists to catch.',
            review: '2099-01-01',
        };
        expect(lockEntriesFor(lock, 'uuid').length).toBeGreaterThan(0);
        expect(stableDormantFloors([...DORMANT_FLOORS, falseClaim], lock)).toEqual(['uuid']);
    });
    ...
    it('datedEntries really is both lists — the selection `expired` reasons over', () => {
        // `if (1) return [];` in datedEntries() retires the entire dated
        // fuse: 'no expired entry' reads an empty selection and passes, and
        // #866 measured that gut staying green even with a waiver back-dated
        // to 2020-01-01. An identity assertion against the two source lists
        // is what the emptiness of that selection was missing — it cannot
        // hold while either list has an entry this function drops or
        // re-dates.
        const expectedIds = [
            ...DORMANT_FLOORS.map((d) => `A:${d.key}`),
            ...WAIVERS.map((w) => `${w.check}:${w.target}`),
        ];
        const expectedReviews = [...DORMANT_FLOORS.map((d) => d.review), ...WAIVERS.map((w) => w.review)];
        expect(datedEntries().map((e) => e.id)).toEqual(expectedIds);
        expect(datedEntries().map((e) => e.review)).toEqual(expectedReviews);
    });

    it('every REAL dated entry is expired under a clock past every review date', () => {
        // The other half, and the one that proves the wiring rather than the
        // bookkeeping: `expired` run over the live `datedEntries()` — no
        // fixture anywhere — with the clock moved past every review date in
        // the file. Every entry must come back. A gutted datedEntries()
        // returns [] here and fails; so does an `expired` that stops
        // comparing. The synthetic fixtures further down prove the BOUNDARY
        // (an entry is due, not overdue, on its own review date); this proves
        // the rule reaches the entries this file actually carries.
        const overdue = expired(new Date('2099-01-01T00:00:00Z'));
        expect(overdue).toHaveLength(datedEntries().length);
        expect(overdue.some((e) => e.startsWith('A:hono '))).toBe(true);
    });
...
describe('overrides — the guard turns red on the REAL tree, once per check', () => {
    ...
    //
    // ## The re-derived number, since one was asked for (#866)
    //
    // Re-measured on 2026-09-10 against this branch's own tree: 34 override
    // keys, 46 edges, **exactly 20 findings** (A 2, B 13, C 2, D 3), and 18
    // WAIVERS + 2 DORMANT_FLOORS accounting for all 20 with nothing stale.
    // `> 20` was therefore red by one on a tree with nothing wrong with it,
    // and `> 19` — the mechanical repair — would be red by one again the
    // day the next entry is fixed. There is no value of N that is both a
    // floor on today's tree and survives the fix it is asking for, because
    // the quantity is the count of OPEN defects and the work is to drive it
    // to zero.
    //
    // The number that replaced it is per check and it is ONE: each of A, B,
    // C and D must produce at least one finding on the REAL package.json
    // and package-lock.json when a single defect of its own shape is
    // injected — the four `it`s below. One is a floor the fix cannot break:
    // the injected defect is put there by the test, so it is present no
    // matter how clean the tree becomes. The anti-vacuity properties the
    // count was carrying are kept elsewhere and separately: the population
    // floors above (>5 keys, >25 edges, >10 nested, >1000 lockfile
    // entries), which fall only when the overrides table itself is deleted,
    // and 'a substantial set of overrides is covered and CLEAN' below,
    // which fails if every entry ends up waived.
