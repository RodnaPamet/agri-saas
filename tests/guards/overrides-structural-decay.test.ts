 ...
 *
 * ## Why the fixtures — and why a fixture is not a proof
 *
 ...
 * analysis returned nothing at all. So each RULE is driven by a synthetic
 * fixture that must fail, paired with a positive control that must pass.
 *
 * That is necessary and it is NOT sufficient, which is what #866 measured on
 * this file. A fixture proves the rule; it says nothing about the path that
 * executes. The five bookkeeping selectors — `unwaived`, `staleWaivers`,
 * `stableDormantFloors`, `datedEntries`, `expired` — had fixture controls
 * feeding nought or one entry while every live assertion fed the real ~20
 * findings. The two input sets never overlapped, so each of the five could
 * be gutted with the suite still at 45/45 green.
 *
 * So there are two mutation blocks near the end, and both run on the REAL
 * package.json + package-lock.json with exactly one defect injected:
 *
 *   - 'the guard turns red on the REAL tree, once per check' — the four
 *     CHECKS, one proof each, because vacuity is per check.
 *   - 'the bookkeeping itself has teeth' — the five SELECTORS, each fed the
 *     real findings and the real lists with one element patched, dropped or
 *     added.
 *
 * That is the standard `tests/guards/dependency-policy-claims.test.ts` sets
 * on the real parsed document: a checker fed a fixture proves nothing about
 * the input the production tree produces.
 */
...
// ─────────────────────────────────────────────────────────────────────────
// Bookkeeping selectors over the two lists.
//
// Every one takes its population as a PARAMETER defaulting to the live list.
// That is not decoration, it is the whole repair (#866). Until then these
// read the module-level lists directly and their only controls fed a
// hand-made fixture of nought or one entry, while every live assertion fed
// the real findings. Measured on this tree, one gut each:
//
//     unwaived             if (findings.length > 1) return [];    45 passed
//     staleWaivers         if (findings.length > 0) return [];    45 passed
//     stableDormantFloors  return [];                             45 passed
//     datedEntries         return [];                             45 passed
//     expired              if (entries.length > 1) return [];     45 passed
//
// Two of those are not even keyed on a size: `stableDormantFloors` and
// `datedEntries` took no argument, so there was no fixture to feed them at
// all and a bare `return []` was invisible.
//
// The parameters are what let the proofs at the bottom of this file feed the
// REAL findings and the REAL lists with one element patched, dropped or
// added — the shape tests/guards/dependency-policy-claims.test.ts uses on
...
function waiverKeys(waivers: readonly Waiver[]): Set<string> {
    return new Set(waivers.map((w) => `${w.check}:${w.target}`));
}

function dormantKeys(dormant: readonly DormantFloor[]): Set<string> {
    return new Set(dormant.map((d) => `A:${d.key}`));
}
...
export function unwaived(
    ...
    dormant: readonly DormantFloor[] = DORMANT_FLOORS,
): Finding[] {
    const accepted = new Set([...waiverKeys(waivers), ...dormantKeys(dormant)]);
    return findings.filter((f) => !accepted.has(findingKey(f)));
}

/** Waivers whose finding is no longer produced — a blind spot, not a saving. */
function staleWaivers(findings: Finding[], waivers: readonly Waiver[] = WAIVERS): string[] {
...
function stableDormantFloors(dormant: readonly DormantFloor[] = DORMANT_FLOORS): string[] {
    return dormant
        .filter((d) => {
...
function datedEntries(
    ...
    return [
        ...dormant.map((d) => ({ id: `A:${d.key}`, review: d.review })),
 ...
 *
 * `entries` defaults to the live lists, and the proofs at the bottom drive
 * BOTH halves of that: the live population with only the CLOCK moved — which
 * is byte-for-byte the input `no expired entry` feeds — and the live
 * population with one real entry back-dated. The synthetic pair further down
 * pins the boundary semantics and nothing else; on its own it never touched
 * the executing path, which is #866.
 *
...
describe('overrides — the analysis is looking at something', () => {
    ...
    it('every range in the tree parsed — nothing was skipped silently', () => {
        ...
        // from being a silent shrug. BOTH sides are collected — a requester's
        // declared range and an override's own value — and the second half
        // was missing until #866; see the two proofs in the mutation block.
    ...
    it('every override value resolves to a parseable range', () => {
        // Two ways for a value to be useless, and this test asserted only the
        // first until #866 — while carrying a name that promised both:
        //
        //   1. a `$name` that resolves to nothing (`resolved === null`), and
        //   2. a literal npm cannot parse, which C and D each `continue`
        //      past. `"fast-uri": "not-a-parseable-range"` in the real
        //      package.json passed this file 45/45.
        //
        // The analysis now collects (2) into `unparseableRanges`; the filter
        // below names that half explicitly so the two failures read apart.
        const unresolved = analysis.edges.filter((e) => e.resolved === null);
        expect(unresolved.map((e) => `${e.key} > ${e.child} = ${e.raw}`)).toEqual([]);
        expect(analysis.unparseableRanges.filter((r) => r.includes('override VALUE'))).toEqual([]);
    });
});
...
describe('overrides — every structural finding is answered', () => {
    ...
    // The control for the line above is NOT here any more, and that is the
    // point of #866. It used to be `staleWaivers([])` — nought findings,
    // against a live assertion that always passes the real twenty. Both
    // `return []` and `if (findings.length > 0) return []` walked straight
    // through it. The replacement is in 'the bookkeeping itself has teeth'
    // at the bottom of this file, where the same call gets the real findings
...
describe('overrides — the guard turns red on the REAL tree, once per check', () => {
    ...
    it('an override VALUE npm cannot parse is collected, not dropped', () => {
        // #866's sharpest defect, and the only one needing no guard mutation
        // at all: this exact injection into the real package.json left this
        // file AND tests/guards/overrides-no-direct-dep-conflict.test.ts at
        // 50 passed / 50 — including the test named 'every override value
        // resolves to a parseable range'. `resolve()` hands back the raw
        // literal, and C and D both `continue` on a value they cannot parse,
        // so the entry was not analysed by anything.
        const mutated: PackageJson = {
            ...pkg,
            overrides: { ...pkg.overrides, 'fast-uri': 'not-a-parseable-range' },
        };
        const collected = analyseOverrides(mutated, lock).unparseableRanges;
        expect(collected).toEqual([expect.stringContaining('not-a-parseable-range')]);
        expect(collected[0]).toContain('fast-uri');
        // ...beside the un-mutated tree, which collects none. Without this
        // the assertion above is satisfied by a list that is always full.
        expect(analysis.unparseableRanges).toEqual([]);
    });

    it('a `$name` override pointing at nothing is reported, not resolved to itself', () => {
        // The one-word defensive edit `?? null` → `?? raw` in overrideEdges'
        // `resolve` hides this. It is now caught twice: as an unresolved edge
        // here, and — once `raw` is handed back in place of null — as an
        // unparseable override VALUE by the proof above.
        const mutated: PackageJson = { ...pkg, overrides: { ...pkg.overrides, sharp: '$sharp-typo' } };
        const { edges } = analyseOverrides(mutated, lock);
        expect(edges.filter((e) => e.resolved === null).map((e) => e.key)).toEqual(['sharp']);
        expect(analysis.edges.filter((e) => e.resolved === null)).toEqual([]);
    });
...
describe('the checks themselves — synthetic fixtures that MUST fail', () => {
    ...
    describe('the waiver bookkeeping — the RULES, on synthetic entries', () => {
        // These pin the boundary SEMANTICS and nothing else, and they keep
        // pinning them after the last waiver is retired. They are NOT the
        // control for the live assertions: a one-entry fixture against a
        // fifteen-entry live list is exactly the gap #866 measured, and the
        // real-input proofs are in the block below this one.
...
// ─────────────────────────────────────────────────────────────────────────
// The BOOKKEEPING itself has teeth — on the REAL findings (#866).
//
// The block above proves the four CHECKS fire on the real tree. This one
// proves the five selectors that decide what to DO with a finding fire on
// it too, and that is the half that was missing: every control they had fed
// a hand-made population of nought or one while the live assertions fed the
// real findings, so all five could be gutted at 45/45 green.
//
// So no fixture is hand-built here. Each case is the real `analysis`, the
// real WAIVERS and the real DORMANT_FLOORS with exactly ONE element patched,
// dropped or added — the standard
// tests/guards/dependency-policy-claims.test.ts sets on the real parsed
// document: "a checker fed a fixture proves nothing about the input the
// production tree produces."
//
// The un-mutated controls are the live assertions in 'every structural
// finding is answered' above. Each case below is the same call, one defect
// in, and the pair is what makes either half mean anything.
// ─────────────────────────────────────────────────────────────────────────
describe('the bookkeeping itself has teeth — on the REAL findings', () => {
    const liveKeys = new Set(analysis.findings.map(findingKey));

    /**
     * The denominator, asserted before anything below selects over it.
     *
     * Every proof here patches or drops one element of the real population,
     * so an empty population would make all of them vacuous — the "an empty
     * selection is a PASS" shape, one level up from the one the four checks
     * guard against.
     *
     * This is NOT the `findings.length > 20` floor that was removed from the
     * block above, and the difference is the failure each produces. That one
     * went red when somebody FIXED an entry, and the only way to clear it was
     * to lower the number. This one goes red only when a list reaches ZERO,
     * and then the instruction is not "lower it" but "this rule has no
     * subject left — retire the machinery and these proofs with it". Both
     * lists are supposed to get there.
     */
    it('the real population these proofs mutate is not empty', () => {
        expect({
            findings: analysis.findings.length > 0,
            waivers: WAIVERS.length > 0,
            dormantFloors: DORMANT_FLOORS.length > 0,
        }).toEqual({ findings: true, waivers: true, dormantFloors: true });
    });

    /** A waiver the real tree really does still produce a finding for. */
    function aLiveWaiver(): Waiver {
        const found = WAIVERS.find((w) => liveKeys.has(`${w.check}:${w.target}`));
        if (!found) {
            throw new Error(
                'no WAIVERS entry matches a live finding, so these proofs are mutating nothing. ' +
                    'That state is itself a failure — see the no-stale-waiver assertion above.',
            );
        }
        return found;
    }

    /** A dormant floor the real tree really does still produce an A finding for. */
    function aLiveDormantFloor(): DormantFloor {
        const found = DORMANT_FLOORS.find((d) => liveKeys.has(`A:${d.key}`));
        if (!found) throw new Error('no DORMANT_FLOORS entry answers a live check-A finding');
        return found;
    }

    /**
     * The earliest review date in the LIVE list — a clock on which nothing
     * real is yet overdue.
     *
     * Derived rather than written down, so the two back-dating proofs keep
     * isolating their own injected entry after the fuse in WAIVERS burns:
     * fifteen entries fall due on one day, and a hard-coded `new Date()`
     * would make those proofs go red alongside `no expired entry` instead of
     * continuing to say whether the SELECTOR still works.
     */
    function beforeAnythingIsDue(): Date {
        const entries = datedEntries();
        const earliest = entries.reduce((a, b) => (a.review <= b.review ? a : b));
        return new Date(`${earliest.review}T12:00:00Z`);
    }

    it('unwaived — withdrawing a REAL waiver leaves its finding open', () => {
        const w = aLiveWaiver();
        const id = `${w.check}:${w.target}`;
        const open = unwaived(
            analysis.findings,
            WAIVERS.filter((x) => x !== w),
            DORMANT_FLOORS,
        );
        expect(open.map(findingKey)).toEqual([id]);
    });

    it('unwaived — withdrawing a REAL dormant floor leaves its check-A finding open', () => {
        // Check A is answered only by DORMANT_FLOORS, so a selector reading
        // WAIVERS alone would pass every other case in this block.
        const d = aLiveDormantFloor();
        const open = unwaived(
            analysis.findings,
            WAIVERS,
            DORMANT_FLOORS.filter((x) => x !== d),
        );
        expect(open.map(findingKey)).toEqual([`A:${d.key}`]);
    });

    it('unwaived — a NEW finding arriving beside the real ones is not absorbed', () => {
        const extra: Finding = { check: 'C', target: 'a-package-no-waiver-names', detail: 'injected' };
        expect(unwaived([...analysis.findings, extra]).map(findingKey)).toEqual([findingKey(extra)]);
    });

    it('staleWaivers — a REAL waiver whose finding stops being produced reads as stale', () => {
        const w = aLiveWaiver();
        const id = `${w.check}:${w.target}`;
        const without = analysis.findings.filter((f) => findingKey(f) !== id);
        // The size of the mutation, printed beside its verdict: "reported
        // nothing" and "was handed the same input" are otherwise identical.
        expect({ removed: analysis.findings.length - without.length }).toEqual({ removed: 1 });
        expect(staleWaivers(without, WAIVERS)).toEqual([id]);
    });

    it('staleWaivers — a waiver stranded on the real tree reads as stale', () => {
        // How the eleven @typescript-eslint / picomatch waivers were stranded
        // when their dependency fix landed first: the finding stopped being
        // produced and each waiver silently became a blind spot. The gut that
        // survived review — `if (findings.length > 0) return []` — is exactly
        // the one this case kills, because it feeds the real twenty.
        const stranded: Waiver = {
            check: 'D',
            target: 'picomatch',
            reason:
                'Injected by this proof only. The real picomatch floor was raised to ^4.0.7 before ' +
                'this guard landed, so no D finding is produced for it and a waiver naming one is stale.',
            review: '2027-01-01',
        };
        expect(staleWaivers(analysis.findings, [...WAIVERS, stranded])).toEqual(['D:picomatch']);
    });

    it('stableDormantFloors — a REAL dormant floor re-pointed at an INSTALLED key is reported', () => {
        // A dormant floor is a claim that the package is ABSENT. `uuid` is an
        // override key the lockfile definitely resolves, so re-pointing one
        // real entry at it makes the claim false — the shape that arrives for
        // real when a dropped transitive edge comes back.
        expect({ uuidIsInstalled: lockEntriesFor(lock, 'uuid').length > 0 }).toEqual({
            uuidIsInstalled: true,
        });
        const target = DORMANT_FLOORS[0];
        const patched = DORMANT_FLOORS.map((d) => (d === target ? { ...d, key: 'uuid' } : d));
        expect(stableDormantFloors(patched)).toEqual(['uuid']);
    });

    it('datedEntries — carries every REAL waiver AND every REAL dormant floor', () => {
        const ids = datedEntries().map((e) => e.id);
        expect(ids).toHaveLength(DORMANT_FLOORS.length + WAIVERS.length);
        for (const d of DORMANT_FLOORS) expect(ids).toContain(`A:${d.key}`);
        for (const w of WAIVERS) expect(ids).toContain(`${w.check}:${w.target}`);
    });

    it('datedEntries + expired — a back-dated REAL entry is caught, from EITHER list', () => {
        // Dropping one half of `datedEntries` is invisible to a length check
        // alone once the other half is big enough, so each list is back-dated
        // in turn and the id that comes back names which one was read.
        const clock = beforeAnythingIsDue();

        const w = WAIVERS[0];
        const backDatedWaivers = WAIVERS.map((x) => (x === w ? { ...x, review: '2020-01-01' } : x));
        expect(expired(clock, datedEntries(DORMANT_FLOORS, backDatedWaivers))).toEqual([
            `${w.check}:${w.target} (review was 2020-01-01)`,
        ]);

        const d = DORMANT_FLOORS[0];
        const backDatedFloors = DORMANT_FLOORS.map((x) => (x === d ? { ...x, review: '2020-01-01' } : x));
        expect(expired(clock, datedEntries(backDatedFloors, WAIVERS))).toEqual([
            `A:${d.key} (review was 2020-01-01)`,
        ]);
    });

    it('expired — the LIVE list, with only the clock moved past its earliest review', () => {
        // Byte-for-byte the input 'no expired entry' feeds: the default
        // `datedEntries()`, no argument, no fixture. The only thing moved is
        // the one input a guard does not control. On the review date itself
        // the entry is due, not overdue; the day after it is reported.
        const entries = datedEntries();
        const earliest = entries.reduce((a, b) => (a.review <= b.review ? a : b));
        const onTheDay = beforeAnythingIsDue();
        const dayAfter = new Date(onTheDay.getTime() + 86_400_000);
        expect(expired(onTheDay)).toEqual([]);
        expect(expired(dayAfter)).toContain(`${earliest.id} (review was ${earliest.review})`);
    });

    it('describeFindings — the failure message renders a REAL finding', () => {
        // Only ever rendered on the failing path, so gutting it to '' costs
        // nothing at merge time and everything at 3am: 'no unwaived finding'
        // would report a count and no names.
        const f = analysis.findings[0];
        const rendered = describeFindings([f]);
        expect(rendered).toContain(`[${f.check}]`);
        expect(rendered).toContain(f.target);
        expect(rendered).toContain(f.detail);
    });
});
