...
export interface UpdatePlantingInput {
    /**
     * Expected yield, PER HECTARE — the stored/wire representation.
     * The UI enters/displays кг/дка and converts at its own boundary via
     * `haToDca`/`dcaToHa` (`src/lib/agro/rate-calc.ts`) before calling
     * this usecase; the API never speaks decares, same as every other
     * area figure in this schema. `null` clears a previously-set value
     * — it is NOT the same as `0` (see the field's schema doc); a
     * calculator reading this field back must treat `null` as an
     * explicit exclusion, never coerce it to zero.
     */
    plannedYieldKgPerHa?: number | null;
}

/**
 * Update planting-level fields the succession engine does not own —
 * today just the grower-entered planned yield. Deliberately narrow:
 * dates/seed/plant-count are engine outputs regenerated by
 * `generatePlantings`, so this usecase does not touch them.
 */
export async function updatePlanting(ctx: RequestContext, id: string, input: UpdatePlantingInput) {
    assertCanWrite(ctx);
    const data: Prisma.PlantingUncheckedUpdateInput = {};
    if (input.plannedYieldKgPerHa !== undefined) data.plannedYieldKgPerHa = input.plannedYieldKgPerHa;
    ...
    return runInTenantContext(ctx, async (db) => {
        ...
        if (!existing) throw notFound('Planting not found');

        const planting = await db.planting.update({ where: { id }, data });
        await logEvent(db, ctx, {
            action: 'UPDATE',
            entityType: 'Planting',
            entityId: id,
            details: 'Planting updated',
            detailsJson: {
                category: 'entity_lifecycle',
                entityName: 'Planting',
                operation: 'updated',
...
export interface PlantingProgressRow {
    ...
    areaM2: number | null;
    /**
     * Grower-entered expected yield, per hectare. `null` means "no
     * expectation set" — NOT "expect zero" (see the field's schema doc
     * and `summarizePlannedYield` in `@/lib/planning/planned-yield`,
     * which any consumer of this row array pipes through to total the
     * plan's expected output while excluding, rather than zeroing, rows
     * like this one).
     */
    plannedYieldKgPerHa: number | null;
}
...
export async function getCropPlanProgress(ctx: RequestContext, cropPlanId: string): Promise<PlantingProgressRow[]> {
    ...
    return runInTenantContext(ctx, async (db) => {
        ...
        return plantings.map((p) => ({
            ...
            areaM2: dec(p.areaM2),
            // Never coerced to 0 here — dec() returns null straight through
            // for an unset value (see PlantingProgressRow's doc + the
            // netTonnesStd precedent this mirrors).
            plannedYieldKgPerHa: dec(p.plannedYieldKgPerHa),
        }));
    });
}
