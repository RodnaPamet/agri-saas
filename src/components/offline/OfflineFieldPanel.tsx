...
export function OfflineFieldPanel({ taskId }: { taskId: string }) {
    ...
    const { online, pending, readable, pendingPhotos, queueGrowing, foreign, durability, submit, flush, conflicts, resolveConflict } = useOfflineSync();
    ...
    return (
        ...
        <FadeIn className="mx-auto max-w-xl space-y-default p-4">
            ...
            <OfflineSyncBar online={online} pending={pending} readable={readable} pendingPhotos={pendingPhotos} queueGrowing={queueGrowing} foreign={foreign} storagePersisted={durability?.persisted ?? null} onSyncNow={() => void flush()} />
