...
| `picomatch` → `^4.0.7` | CVE-2026-33671 — ReDoS in pattern compilation (high) | Bare, not scoped: every requester in the tree must land on the patched 4.x line, and the two `^2.x` requesters (`anymatch`, `micromatch`) are glob matchers whose picomatch surface is version-stable across the major. The floor shipped at `^4.0.4` and had DECAYED: `lint-staged` declares `picomatch: ^4.0.7`, so `^4.0.4` excluded nothing it could reach and additionally ADMITTED versions lint-staged itself rejects — the lockfile resolved 4.0.5 against that `^4.0.7` edge. `npm audit` cannot see this (the resolved version is patched) which is why it sat. Do not drop this entry — and the reason is the production tree, not a suppression. picomatch is a PRODUCTION dependency: `npm ls picomatch --omit=dev` reaches it through `@sentry/nextjs` → `@rollup/plugin-commonjs` → `@rollup/pluginutils` / `fdir`, and through `next-intl` → `@parcel/watcher`. The hoisted copy is `dev: false` in the lockfile, so it survives `npm prune --omit=dev` (Dockerfile:85) and ships in the image Trivy scans (Dockerfile:171). No production requester floors it at the patched line — the highest is `@parcel/watcher`'s `^4.0.4`, with both `@rollup/*` at `^4.0.2` and `fdir` at `^3 \|\| ^4`; the sole `^4.0.7` requester is `lint-staged`, which is dev-only and is pruned out of the image. This override is therefore the only thing holding the SHIPPED copy on the patched line, and no exemption would mute the finding if it slipped: `.trivyignore` carries nothing for this id (`grep -c 33671 .trivyignore` → 0; the file reads "No active exemptions"), because 87af96901 (#647) deleted all eight entries. The id WAS exempted once, on the argument that the only vulnerable copy lived inside the npm CLI — that entry and that argument are both gone — see `tests/guards/trivyignore-exemptions.test.ts`. |
