...
|----------|-------------------|-------|------------|
| `@visx/*@3.x` vs React 19 | `@visx/axis` → `{react: $react}`, `@visx/clip-path` → `{react: $react}`, `@visx/event` → `{react: $react}`, `@visx/gradient` → `{react: $react}`, `@visx/group` → `{react: $react}`, `@visx/responsive` → `{react: $react}`, `@visx/shape` → `{react: $react}`, `@visx/text` → `{react: $react}`, `@visx/tooltip` → `{react: $react, react-dom: $react-dom}` | visx 3.x (the latest stable line) peers `react ^16 \|\| ^17 \|\| ^18`; visx 4 — which adds React 19 — is alpha-only. The repo runs `react@19`. | `overrides` block: each `@visx/*` package's `react` / `react-dom` pinned to the root version (`$react` / `$react-dom`). visx 3.x is a set of stateless SVG renderers and runs correctly under React 19 — the override records that verified fact. **Twelve of the original 22 subkeys were inert and always had been**, which the structural guard described under "Structural decay" below measures: among those keys only `@visx/tooltip` declares a `react-dom` peer (`@visx/bounds` declares one too, but has no override key), and `@visx/curve` / `@visx/scale` — d3 wrappers with no JSX — declared no `react` peer either. npm scopes a nested override to the parent's own declared edge, so for those twelve there had never been an edge to rewrite. They were DELETED rather than renewed when their waivers came due, taking `@visx/curve` and `@visx/scale` out entirely (every subkey of each was dead) and leaving eight keys pinning `react` alone. The **live** pinning is `react` on eight packages plus `react` and `react-dom` on `@visx/tooltip`; that is what carries React 19. Deleting them changed no resolved version, which is what the waivers predicted and what the deletion PR measured. |
...
|-------|------|---------------------------|
| **A — no floor without a target** | Every top-level override key must match at least one lockfile entry, or be listed as a **dormant floor** with a written reason. | `hono` guards a package absent from the lockfile, so it is invisible to `npm audit` AND to Dependabot. That is how it decayed twice with nothing going red. Its sibling `@hono/node-server` was deleted rather than renewed: no advisory was ever recorded for it, and a floor whose target nobody can name is decoration, not protection. |
...
something older, plus `picomatch` on D — and that fix landed first, so
what ships is **20 findings over 34 entries** (A 2, B 13, C 2, D 3),
with all four checks still red. It therefore ships with an explicit
waiver list (`WAIVERS` and `DORMANT_FLOORS` in the guard), one entry per
remaining finding, each carrying a written reason and a `review` date.
It borrows both sharp rules from `scripts/audit-exemptions.mjs`: a
**stale** waiver fails the build (the finding stopped being produced, so
the waiver is now a blind spot), and an **expired** waiver fails the
build once its own author's review date passes. The list can therefore
only shrink. Sixteen of the 34 keys carry no waiver at all, so a new
finding on any of those fails the build with nothing standing between it
and the reader.

**How the guard proves it still works.** A waiver list is a selection,
and an empty selection passes every assertion over it — so the guard
does not merely assert that its own bookkeeping found nothing. Two
mutation blocks run against the **real** `package.json` +
`package-lock.json` with one defect injected: one shows each of the four
checks going red on this tree, and one feeds each bookkeeping selector
the real findings with a single waiver withdrawn, stranded or
back-dated. The pattern is the one
`tests/guards/dependency-policy-claims.test.ts` uses on the real parsed
document — a checker fed a hand-made fixture proves nothing about the
input the production tree produces — and #866 is the measurement that
forced it. With fixture-only controls, all five selectors survived a gut:
two of them (`stableDormantFloors`, `datedEntries`) took no argument at
all, so a bare `return []` was invisible, and the other three fell to a
one-line gut keyed on the size of the input — the fixture passed one
entry, the live assertion passed twenty.
...
reviewer's job. And this whole section is prose: it is covered by the
"UNGUARDED PROSE" disclaimer above like every other narrative here, so
read it, do not trust it because CI is green.
