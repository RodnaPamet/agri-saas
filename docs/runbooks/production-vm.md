...
| Prometheus, Grafana, Alertmanager, PagerDuty **running anywhere** | `infra/alerts/receivers.yml` and `infra/dashboards/` are files in this repo describing a Kubernetes stack this product does not have (#856). The VM runs seven containers and none is an observability component; `/opt/agrent/.env` carries no OTLP, Sentry or PagerDuty key. **What DOES exist since 2026-09-17 (#854): a GCP Cloud Monitoring uptime check** — `agrent-readyz-oKY0R5q09QU`, probing `https://app.agrent.bg/api/readyz` every 60s from six regions, requiring a 2xx body containing `"status":"ready"` — and alert policy `agrent production is not ready (#854)`, which fires on sustained failure from more than one region. So detection latency is bounded at about two minutes. An email notification channel is attached (`agrent on-call`) and was proved to deliver on 2026-09-17. **There is still no rota and no pager** — one address, one person, email — so an outage at 03:00 waits for someone to read their inbox. Detection is bounded; response is not. `infra/alerts/external-uptime.yml` carries the detail, including why the probe is `readyz` and not `livez`. |
