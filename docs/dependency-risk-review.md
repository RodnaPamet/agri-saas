...
| **Classification** | `devDependencies` — **moved.** It was `dependencies` while three server modules parsed YAML; all three are gone (`mapping-set-importer.ts` phase 2, `prisma/catalog-loader.ts` phase 3, `library-loader.ts` with the framework-library subsystem). Nothing in `src/` imports it, and `npm ls js-yaml --omit=dev` shows no transitive production dependent, so the move strips it from the production image cleanly. `tests/guards/dependency-risk-review.test.ts` asserts the classification in BOTH directions now — a dev-only reviewed package reappearing in `dependencies` is as much a regression as a runtime one leaking out — and additionally fails if any `src/` file starts importing it again. |
