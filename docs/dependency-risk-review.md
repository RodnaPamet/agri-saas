...
| **Runtime use** | **NONE in `src/` as of the framework-library deletion.** The only remaining consumers are `tests/guards/*` (workflow / helm / terraform YAML lint). Every production call site this row has ever named is now gone: `mapping-set-importer.ts` with GRC teardown phase 2, `prisma/catalog-loader.ts` with phase 3, and `src/app-layer/libraries/library-loader.ts` with the framework-library subsystem it belonged to. |
| **Classification** | `dependencies` — **NO LONGER JUSTIFIED, and deliberately not changed here.** The previous revision of this row said: *"If that subsystem is deleted, re-check whether `js-yaml` still belongs in `dependencies`."* It has been deleted, and the answer is that nothing in `src/` imports it. Moving it to `devDependencies` is therefore the correct verdict — but it removes a package from the production image, so it is a deploy-affecting change that belongs in its own PR with its own verification, not bundled into a code deletion. `tests/guards/dependency-risk-review.test.ts` must move in the same diff: its `REVIEWED` table documents "every entry here was proven to be runtime-needed", which is now false for this one package. |
