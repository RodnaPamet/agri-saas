...
| `20260815120000_grc_teardown_phase3.down.sql` | The GRC schema drop (47 tables, 44 enums, 148 indexes, 119 FKs, 5 columns on live agri tables, 4 narrowed enums). Restores the SHAPE the previous image's Prisma client expects; it cannot restore row DATA (all 47 tables held 0 rows in production) and does not restore RLS on the recreated tables — see the script header, which says why a red `rls-coverage` guard after a rollback is the correct signal. **Ordering:** run this BEFORE `…_rename_control_to_practice.down.sql`, which renames a `NotificationType` value phase 3 removes. | Executed against Postgres 16 from the same image the test DB uses: 238-migration history applied, phase 3 applied, this script run (47/47 tables back, `Evidence.practiceId` back, all four checked enum values back, `_prisma_migrations` row gone), then `migrate deploy` rolled forward again. Residual drift 31 statements — the repo's pre-existing baseline, unchanged. |
